...
export default class Timer {
	...
	toJSON() {
		...
		for (let i = 0; i < this.tasks.length; i++) {
			taskProperties.push(this.tasks[i].toJSON());
		}
	...
	static rebuildObject(properties) {
		for (let i = 0; i < properties.tasks; i++) {
			Object.setPrototypeOf(properties.tasks[i], Task.prototype);
		}

		return Object.setPrototypeOf( properties, Timer.prototype);
	}
...
class Task {
	...
	toJSON() {
		return {
			...
			intervalTimer: this.intervalTimer,
			totalTime: this.totalTime,
			duration: this.duration,
			callback: JSON.stringify(this.callback),
			isDone: this.isDone,
			className: this.constructor.name
		}
	}
